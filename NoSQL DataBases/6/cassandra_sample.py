...
print('Example 1 - Insert population information in table with columns defined for each field')
...
print('Example 2 - Using multiple search filters in where clause')
# Select where city is Cupertino and county is Santa Clara
city_stmt = session.prepare("Select * from city_population where city=? and county=?")
rows = session.execute(city_stmt, ['Cupertino', 'Santa Clara'])

print('Select city:Cupertino and county:Santa Clara')
for row in rows:
    print(row)
    print('City Name :', row.city, ', City Population:', row.population)

print('Example 3 - Using aggregate functions')
